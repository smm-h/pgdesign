...
- PRESENCE IS DECLARED: every flag and every positional arg states exactly one of `strictcli.Required()`, `strictcli.Optional()` or `strictcli.Default(v)` (args: `ArgRequired()` / `ArgOptional()` / `ArgDefault(v)`); declaring none or two is a registration-time hard error, and `Default(nil)` is refused with a redirect to `Optional()`.
- NO MUTATING DEFAULT: on a command declaring `EffectMutating`, no flag and no arg may declare a value default — absence must never resolve to a value the invocation did not state. Every such switch declares `Optional()` and NAMES its fallback in its own help text; `optBool`/`optStr`/`optInt` in `cmd/pgdesign/globals.go` are the only place where absence becomes that fallback. Declared defaults stay legal on `read_only` commands (`generate`, `serve`), which is why `--format sql` and `--port 8080` still have them.
- CHOICES ARE RECORDS: every `Choices(...)` entry is `strictcli.Ch(value, help)`; a bare value is a registration error. `SupportedModeChoices()` in `codegen_registry.go` derives `codegen --mode`'s records from the one mode registry, so a mode cannot exist without a description.
- NO UPDATE COMMANDS: no pgdesign command declares strictcli's `WithUpdateOf` — nothing here changes some properties of one addressed resource instance and leaves the rest alone. `cmd/pgdesign/classification_test.go` records the reasoning per command family and enforces the verdict against `--dump-schema`.
- `generate` declares `strictcli.OwnsStdout()`: its stdout IS the artifact, so under the framework-owned `--json` the machine envelope goes to stderr and `--format json`'s hash-verified document keeps its exact bytes. `cmd/pgdesign/generate_json_bytepin_test.go` pins that in both modes.

...
func TestCreateTable(t *testing.T) {
	...
	got := CreateTable(table, "blog", false, 0, nil, nil)
...
func TestCreateTable_WithPartitioning(t *testing.T) {
	...
	got := CreateTable(table, "public", false, 0, nil, nil)
...
func TestColumnDef_VirtualVersionGate(t *testing.T) {
	...
	got := CreateTable(base, "public", false, 18, nil, nil)
	...
	got = CreateTable(base, "public", false, 17, nil, nil)
	if !strings.Contains(got, "STORED") {
	...
	got = CreateTable(base, "public", false, 0, nil, nil)
	if !strings.Contains(got, "STORED") {
...
func TestCreateTable_IdentityColumn(t *testing.T) {
	...
	got := CreateTable(table, "public", false, 16, nil, nil)
...
func TestCreateTable_IdentityFallbackPrePG10(t *testing.T) {
	...
	got := CreateTable(table, "public", false, 9, nil, nil)
...
func TestCreateTable_IdentityNoFallbackPG10(t *testing.T) {
	...
	got := CreateTable(table, "public", false, 10, nil, nil)
...
func TestCreateTable_EnumColumnSchemaQualified(t *testing.T) {
	...
	got := CreateTable(table, "game", false, 0, enums, nil)
...
func TestCreateTable_ArrayColumnWithEnum(t *testing.T) {
	...
	got := CreateTable(table, "app", false, 0, enums, nil)
...
func TestIdempotentMode(t *testing.T) {
	...
	got = CreateTable(table, "public", true, 0, nil, nil)
	if !strings.Contains(got, "IF NOT EXISTS") {
...
// TestCreateTable_DomainColumnSchemaQualified mirrors the enum test above:
// domain-backed columns must be emitted schema-qualified so the DDL works
// without relying on search_path. Regression test: domain references used to
...
func TestCreateTable_DomainColumnSchemaQualified(t *testing.T) {
	...
	emailType := typeinfo.T("text")
	emailType.DomainName = "email_addr"
	...
	table := &model.Table{
		...
		Columns: []model.Column{
			{Name: "id", PGType: typeinfo.T("uuid"), NotNull: true},
			{Name: "email", PGType: emailType, NotNull: true},
		},
		PK: []string{"id"},
	}

	got := CreateTable(table, "game", false, 0, nil, domains)

	if !strings.Contains(got, "email game.email_addr NOT NULL") {
		t.Errorf("expected schema-qualified domain type game.email_addr, got:\n%s", got)
	}
	if !strings.Contains(got, "id uuid NOT NULL") {
		t.Errorf("expected plain uuid type, got:\n%s", got)
	}

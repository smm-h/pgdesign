...
| `rollback` | **Rewritten (5.6).** Journal-driven: inverse content and checksums are journal/store-sourced; topology (the parent/target endpoints the position steps through) still reads edge endpoints from disk. Reverse recorded down-ops. Retargeted to a REVISION (see `--to`). |

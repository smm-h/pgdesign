...
//	validate_constraint           | nested-modifier | delta   | non-invertible (default: invClassForKind); the shim upgrades to declared-inverse because generation ALWAYS records a re-validate down (a no-op that runs before the paired NOT-VALID add's drop_fk on reverse-order rollback) [5.9: generation always splits FK adds]
